...
// Register Property Segment Taxonomy (still used in permalink, but not filtered)
...
// Modify the permalink structure to include taxonomy segment
function custom_property_permalink($post_link, $post) {
    if ($post->post_type === 'property') {
        $terms = wp_get_post_terms($post->ID, 'property_region');
        if (!empty($terms) && !is_wp_error($terms)) {
            $segment_slug = $terms[0]->slug;
            return home_url('properties/' . $segment_slug . '/' . $post->post_name);
        }
    }
    return $post_link;
}
...
// Rewrite rule to support the custom permalink
...
// ✅ ADD: Property Location Meta Box
...
// ✅ SAVE: Property Location Meta Field
...
// ✅ LIMIT TO 10 PROPERTIES PER PAGE IN ADMIN BY DEFAULT
...
// ✅ ADD CUSTOM DROPDOWN FOR ITEMS PER PAGE
...
// Add Next/Prev pagination links to Properties admin list
